...
async def _fetch_pair_data(hl_client, symbol: str):
    ...
    tasks = [hl_client.get_candles(symbol, "1m", 30)]
    need_5m  = c5m  is None
    need_15m = c15m is None
    need_1h  = c1h  is None
    if need_5m:  tasks.append(hl_client.get_candles(symbol, "5m",  100))
    if need_15m: tasks.append(hl_client.get_candles(symbol, "15m", 100))
    if need_1h:  tasks.append(hl_client.get_candles(symbol, "1h",  100))
    tasks.extend([hl_client.get_orderbook(symbol, 20), hl_client.get_price(symbol)])
